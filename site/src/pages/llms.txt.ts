...
export async function GET() {
  ...
  const lines = [
    ...
    "",
    `Full corpus (all pages, one document): ${siteUrl("/llms-full.txt")}`,
    "",
    "## Pages",
    "",
  ];
  ...
  for (const leaf of leaves) {
    ...
    rows.push({
      key: leaf.url,
      line: `- [${leaf.title}](${siteUrl(leaf.markdownUrl)})${description}`,
    });
  }
  ...
  for (const group of groups) {
    ...
    rows.push({
      key: `/${group.slug}`,
      line: `- [${group.label}](${siteUrl(`/${group.slug}/llms.txt`)})`,
    });
  }
