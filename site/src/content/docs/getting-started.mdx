...
[deployment](/deployment/) for the container image, environment
...
- **Permissions**: `checks: write`, `actions: write`, `contents: read`,
  ...
  [permissions](/permissions/) for why each one is needed —
...
See the [`.slash/` reference](/slash-config-reference/) for the full
schema, and run `slash validate .slash/` in your own CI to catch mistakes
before they reach the default branch (see
[known limitations](/limitations/) — Slash does not validate `.slash/` on
push itself).
...
- [`.slash/` reference](/slash-config-reference/) — the full
  `.slash/*.yml` schema
- [Workflow requirements](/workflow-requirements/) — injected inputs,
  the `env:`-not-`${{ }}` rule, required-status-check caveats
- [Permissions](/permissions/) and [Grants](/grants/) — who can invoke what
- [Web console](/web-console/) — sign in, create teams, manage the Test Engine
- [Test Engine](/test-engine/) — collect results and quarantine flaky tests
- [Deployment](/deployment/) — running the server, key rotation, incident
  procedure
- [Known limitations](/limitations/) — what Slash deliberately does not do yet
