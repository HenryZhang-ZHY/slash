...
- **No kill switch.** The design calls for a per-installation and global
  ...
  [deployment guide](/en/self-host/deployment/#incident-procedure).
...
- **No `slash_github_api_calls_total`/rate-limit-remaining metric.** Every
  ...
  [deployment guide](/en/self-host/deployment/#observability)), but per-endpoint GitHub API
...
run the server against any Postgres with a known webhook secret (in a file at
`SLASH_WEBHOOK_SECRET_PATH`), then
